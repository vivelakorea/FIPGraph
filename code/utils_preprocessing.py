...
def __get_all_data_fip_avg(graph_dir, fip_dir, texture, num_nfeat, num_efeat):
    '''
    From graph_dir, get all data of node features, edge features, and fip labels
    '''
...
def make_scaler_fip_avg(scaler_dir, graph_dir, fip_dir, texture, num_nfeat, num_efeat):
    fip_data = __get_all_data_fip_avg(graph_dir, fip_dir+'_avg', texture, num_nfeat, num_efeat)

    fip_scaler = StandardScaler()
    fip_scaler.fit(fip_data)

    with open(file=f'{scaler_dir}\\{texture}\\fip_scaler_avg.pickle', mode='wb') as f:
        pickle.dump(fip_scaler, f, protocol=pickle.HIGHEST_PROTOCOL)
